...
export function importMusicXML(theXML: any){
  ...
  abcText = InjectQTag(abcText,"120");
...
function InjectQTag(theTune: string,theTempo: string){
...
function doTitleCaps(title: string){
	...
	function titleCaps(title: string){
		...
		while (true) {
			...
			parts.push( title.substring(index, m ? m.index : title.length)
				...
				.replace(RegExp("^" + punct + small + "\\b", "ig"), function(all: any, punct: any, word: any){
...
function replaceLineBreaks(input: string, lbChar: string) {
  ...
  function splitTextAtLineBreak(input: string, lbChar: string) {
